...
###################################### cmd for WPMT #########################################
def gen_cmd_naive_WPMT(w, a, v, total, ratio, index_path, WPMT): 
    cmd = './build/nicolas -q {} -p {} -w {} -a {} -m {} --number-of-queries {} -r {} -c {} -i {} -n {} -v {} --fence-pointer no > dat_tmp_WPMT/figure_{}_{}_raw_w_{}_ratio_{}_WPMT_{}.dat'.format(WPMT, merge_threshold, w, a, mode,int(total * (1 - ratio)), int(total * ratio), cardinality, index_path, number_of_rows, v, a, 'latency' if (v == 'yes') else 'throughput',w, ratio, WPMT )
    return cmd

def gen_cmd_others_WPMT(w, a, v, total, ratio, index_path, WPMT):
    cmd = './build/nicolas -q {} -p {} -w {} -a {} -m {} --number-of-queries {} -r {} -c {} -i {} -n {} -v {} --fence-pointer yes --fence-length 100000 > dat_tmp_WPMT/figure_{}_{}_raw_w_{}_ratio_{}_WPMT_{}.dat'.format(WPMT, merge_threshold, w, a, mode, int(total * (1 - ratio)), int(total * ratio), cardinality, index_path, number_of_rows, v, a, 'latency' if (v == 'yes') else 'throughput',w, ratio, WPMT)
    return cmd
